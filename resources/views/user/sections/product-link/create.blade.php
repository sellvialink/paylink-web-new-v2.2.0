...
<div class="body-wrapper payment-body-wrapper">
    <div class="custom-card payment-card">
        <div class="payment-type-wrapper">
            <form action="{{ setRoute('user.product-link.store') }}" method="POST" class="payment-form">
                @csrf
                        ...
                        <div class="row">
                            <div class="col-xl-12 form-group">
                                ...
                                <select class="select2-auto-tokenize currency_link w-100" name="currency" required>
                                    ...
                                    @foreach ($currency_data as $item)
                                        <option value="{{ $item->id }}" {{ $product->currency_name == $item->name ? 'selected' : '' }} data-currency_symbol="{{ $item->symbol }}">{{ $item->code. ' ('. $item->name.')' }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-xl-6 col-lg-6 form-group">
                                ...
                                <div class="input-group">
                                    <div class="input-group-text prepend currency_link_symbol">{{ $product->currency_symbol }}</div>
                                    <input type="number" name="price" class="form--control" value="{{ get_amount($product->price) }}" required>
                                </div>
                            </div>
                            <div class="col-xl-6 col-lg-6 form-group">
            ...
            <div class="tab-content" id="nav-tabContent">
                    ...
                    <div class="payment-preview-mockup">
                        ...
                        <div class="payment-preview-box two">
                            <div class="payment-preview-box-left">
                                <h3 class="sub-title "><i class="lab la-windows"></i> <span class="link-sub-title">{{ @$product->product_name }}</span> <span class="paylink_qty">(1)</span></h3>
                                ...
                                <form class="payment-preview-box-left-form">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <div class="input-group-text prepend currency_link_symbol">{{ $product->currency_symbol }}</div>
                                            <input type="text" class="form--control paylink_amount" value="{{ get_amount($product->price) }}" min="0.1" readonly>
                                        </div>
                                    </div>
                                </form>
                ...
                <div class="tab-pane fade show active" id="web" role="tabpanel" aria-labelledby="web-tab">
                    <div class="payment-preview-box">
                        <div class="payment-preview-box-left">
                            <span class="sub-title"><i class="lab la-windows"></i> <span class="link-sub-title">{{ @$product->product_name }}</span> <span class="paylink_qty">(1)</span></span>
                            @if (!empty($product->desc))
                                <p>{{ $product->desc }}</p>
                            @endif
                            <form class="payment-preview-box-left-form">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-text prepend currency_link_symbol">{{ $product->currency_symbol }}</div>
                                        <input type="text" class="form--control paylink_amount" value="{{ get_amount($product->price) }}" placeholder="0.00" min="0.1" readonly>
                                    </div>
                                </div>
                            </form>
                            <div class="payment-preview-thumb">
