    ...
    public function store(Request $request){
        ...
        if($request->type == PaymentGatewayConst::LINK_TYPE_PAY){
            $validator = Validator::make($request->all(), [
                ...
                'limit'           => 'nullable',
                'min_amount'      => $request->filled('limit') ? 'required_if:limit,==,on|numeric|min:0.1' : '',
                'max_amount'      => $request->filled('limit') ? 'required_if:limit,==,on|numeric|gt:min_amount' : '',
        ...
        return redirect()->route('user.payment-link.share', $payment_link->id)->with(['success' => [__('Payment Link Created Successful')]]);
    }


    /**
     * Payment link edit page show
     *
     * @method GET
     * @return Illuminate\Http\Request
     */
    ...
    public function pagaditoSuccess(){
        ...
        try{
            ...
            if($checkTempData['data']->type == PaymentGatewayConst::TYPEPAYLINK){
                $payment_link = PaymentLink::find($checkTempData['data']->validated->target);
                return redirect()->route('payment-link.transaction.success', $payment_link->token)->with(['success' => [__('Transaction Successful')]]);
            }elseif($checkTempData['data']->type == PaymentGatewayConst::TYPEPRODUCT){
