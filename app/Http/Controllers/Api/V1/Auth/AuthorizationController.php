...
class AuthorizationController extends Controller
{
    ...
    public function emailResendCode(Request $request)
    {
        ...
        if($resend){
            if(Carbon::now() <= $resend->created_at->addMinutes(GlobalConst::USER_PASS_RESEND_TIME_MINUTE)) {
                $error = ['error'=>[__('You can resend verification code after').' '.Carbon::now()->diffInSeconds($resend->created_at->addMinutes(GlobalConst::USER_PASS_RESEND_TIME_MINUTE)). ' seconds']];
    ...
    public function verifyEmailCode(Request $request)
    {
     ...
     public function getKycInputFields() {
        ...
        if(!$user_kyc) return ApiResponse::success(['success' => [__('User KYC section is under maintenance')]], $data);
        ...
        return ApiResponse::success(['success' => [__('User KYC input fields fetch successfully!')]], $data);
    }
